...
function explodeSentence ($breaker, $string, $length, $strip = false, $type = null){
  if ($strip == true) {
  ...
  $explodeString = explode($breaker, $string);
  $returnStringArr =[];

  if ($type == "LAST") {
    for($i = $length; $i < count($explodeString); $i++){
      $returnStringArr[] = $explodeString[$i];
    }
  }else{
  ...
  $returnString =  implode($breaker, $returnStringArr);

  // if (strlen($string) > strlen($returnString)) {
  //   if ( $type == "LAST") {
